  ...
  subCommands: {
    add: () => Promise.resolve(defineCommand({
      ...
      run({ args }) {
        ...
        try {
          const project = obsxa.project.add({ id: args.id, name: args.name, description: args.description })
      ...
      run({ args }) {
        ...
        try {
          ...
          for (const project of projects) {
            consola.log(`${project.id}  ${project.name}${project.description ? ` (${project.description})` : ''}`)
          }
        } finally {
          obsxa.close()
        }
