...
export default defineCommand({
  ...
  subCommands: {
    create: () => Promise.resolve(defineCommand({
    ...
    restore: () => Promise.resolve(defineCommand({
      meta: { name: 'restore', description: 'Restore database from backup base path' },
      args: {
        db: { type: 'string', default: './obsxa.db', description: 'Target SQLite database path' },
        from: { type: 'string', required: true, description: 'Backup base path to restore from' },
        json: { type: 'boolean', default: false, description: 'Output as JSON' },
      },
      run({ args }) {
        try {
