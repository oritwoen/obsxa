...
const observationTypes: ObservationType[] = ['pattern', 'anomaly', 'measurement', 'correlation', 'artifact']
...
const reasonCodes: ObservationStatusReasonCode[] = ['noise', 'duplicate', 'merged', 'irrelevant', 'invalid', 'manual_review', 'promoted']
...
function readDataFile<T>(filePath: string): T {
  try {
    ...
    throw new Error(`Failed to parse data from ${filePath}: ${error instanceof Error ? error.message : error}`)
  }
}
...
export default defineCommand({
  ...
  subCommands: {
    add: () => Promise.resolve(defineCommand({
      ...
      args: {
        ...
        type: { type: 'string', description: 'pattern|anomaly|measurement|correlation|artifact' },
        source: { type: 'string', required: true, description: 'Source label' },
        'source-type': { type: 'string', description: 'experiment|manual|scan|computation|external' },
      ...
      run({ args }) {
        if (args.type && !observationTypes.includes(args.type as ObservationType)) {
          consola.error(`Invalid type "${args.type}". Must be one of: ${observationTypes.join(', ')}`)
          process.exit(1)
        }
        if (args['source-type'] && !sourceTypes.includes(args['source-type'] as SourceType)) {
          consola.error(`Invalid source-type "${args['source-type']}". Must be one of: ${sourceTypes.join(', ')}`)
          process.exit(1)
        }
    ...
    import: () => Promise.resolve(defineCommand({
      meta: { name: 'import', description: 'Import observations from JSON array file' },
      args: {
        ...dbArgs,
        file: { type: 'string', required: true, description: 'Path to JSON file with observation records' },
      },
      run({ args }) {
    ...
    export: () => Promise.resolve(defineCommand({
    ...
    'batch-update': () => Promise.resolve(defineCommand({
      meta: { name: 'batch-update', description: 'Apply batch updates from JSON array file' },
      args: {
        ...dbArgs,
        file: { type: 'string', required: true, description: 'Path to JSON file with update records' },
      },
      run({ args }) {
    ...
    get: () => Promise.resolve(defineCommand({
      ...
      run({ args }) {
        ...
        try {
          ...
          if (args.toon || args.json) return output(observation, args.toon)
          consola.log(`#${observation.id} [${observation.status}] f=${observation.frequency} triage=${observation.triageScore} ${observation.title}`)
        } finally {
          obsxa.close()
        }
      },
    ...
    transitions: () => Promise.resolve(defineCommand({
    ...
    edits: () => Promise.resolve(defineCommand({
      ...
      run({ args }) {
        ...
        try {
          ...
          for (const row of rows) {
            consola.log(`${row.field}: ${row.oldValue ?? '(null)'} → ${row.newValue ?? '(null)'}`)
          }
        } finally {
          obsxa.close()
        }
    ...
    list: () => Promise.resolve(defineCommand({
      ...
      run({ args }) {
        ...
        try {
          ...
          for (const row of rows) {
            consola.log(`#${row.id} [${row.status}] f=${row.frequency} triage=${row.triageScore} ${row.title}`)
          }
        } finally {
          obsxa.close()
        }
    ...
    update: () => Promise.resolve(defineCommand({
    ...
    dismiss: () => Promise.resolve(defineCommand({
      meta: { name: 'dismiss', description: 'Dismiss observation' },
      args: {
        ...dbArgs,
        id: { type: 'positional', required: true, description: 'Observation ID' },
        reason: { type: 'string', required: true, description: `Reason code: ${reasonCodes.join('|')}` },
        note: { type: 'string', description: 'Optional reason note' },
      },
      run({ args }) {
        if (!reasonCodes.includes(args.reason as ObservationStatusReasonCode)) {
          consola.error(`Invalid reason "${args.reason}". Must be one of: ${reasonCodes.join(', ')}`)
          process.exit(1)
        }
    ...
    archive: () => Promise.resolve(defineCommand({
    ...
    bump: () => Promise.resolve(defineCommand({
