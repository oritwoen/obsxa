...
export default defineCommand({
  ...
  args: {
    ...
    ref: { type: 'string', required: true, description: 'Hypothesis reference (e.g. hypxa:project:3)' },
  },
  run({ args }) {
    ...
    try {
      ...
      if (observation.status !== 'active') {
        consola.error(`Observation #${id} must be active to promote (current: ${observation.status})`)
        process.exit(1)
      }
