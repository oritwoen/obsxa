...
export default defineCommand({
  ...
  subCommands: {
      ...
      run({ args }) {
        ...
        try {
          ...
          for (const row of rows) {
            consola.log(`#${row.id} ${row.name}${row.description ? ` (${row.description})` : ''}`)
          }
        } finally {
          obsxa.close()
        }
    ...
    member: () => Promise.resolve(defineCommand({
      ...
      run({ args }) {
        const obsxa = open(args.db)
        try {
          const member = obsxa.cluster.addMember(parseInt(args.cluster, 10), parseInt(args.observation, 10))
    ...
    members: () => Promise.resolve(defineCommand({
