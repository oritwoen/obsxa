...
function toCandidateEvent(row: typeof duplicateCandidateEvents.$inferSelect): DuplicateCandidateEvent {
  return {
    id: row.id,
...
function makeFingerprint(observation: Observation): string {
  const tags = [...observation.tags].map(tag => normalize(tag)).sort().join('|')
  return [
...
export function createDedupStore(db: ObsxaDB) {
  return {
    scan(projectId: string, threshold = 0.72): ScanDuplicatesResult {
      const rows = db.select().from(observations)
      ...
      for (let i = 0; i < rows.length; i += 1) {
        for (let j = i + 1; j < rows.length; j += 1) {
          ...
          if (left.sourceRef && right.sourceRef && left.sourceRef === right.sourceRef) {
            ...
            const titleTokens = jaccard(tokenize(normalize(left.title)), tokenize(normalize(right.title)))
            const titleTrigrams = jaccard(trigrams(normalize(left.title)), trigrams(normalize(right.title)))
            const titleScore = Math.max(titleTokens, titleTrigrams)

            const descTokens = jaccard(tokenize(normalize(left.description)), tokenize(normalize(right.description)))
            const descTrigrams = jaccard(trigrams(normalize(left.description)), trigrams(normalize(right.description)))
            const descriptionScore = Math.max(descTokens, descTrigrams)

            const tagScore = jaccard(new Set(left.tags.map(normalize)), new Set(right.tags.map(normalize)))
            const typeScore = left.type === right.type ? 1 : 0
            const sourceTypeScore = left.sourceType === right.sourceType ? 1 : 0

            const refBoost = (left.sourceRef && right.sourceRef && normalize(left.sourceRef) === normalize(right.sourceRef)) ? 0.15 : 0
            score = (titleScore * 0.40) + (descriptionScore * 0.25) + (tagScore * 0.15) + (typeScore * 0.05) + (sourceTypeScore * 0.05) + refBoost
          ...
          const existing = db.select().from(duplicateCandidates)
            .where(and(
          ...
          if (existing) {
            const updated = db.update(duplicateCandidates)
              .set({
                reason,
                score: storedScore,
                status: 'open',
          ...
          const inserted = db.insert(duplicateCandidates).values({
            projectId,
    ...
    candidates(projectId: string, status: DuplicateCandidateStatus | 'all' = 'open'): DuplicateCandidate[] {
      if (status === 'all') {
        return db.select().from(duplicateCandidates)
    ...
    review(candidateId: number, status: DuplicateCandidateStatus, reason: string): CandidateReviewResult {
      const current = db.select().from(duplicateCandidates).where(eq(duplicateCandidates.id, candidateId)).get()
      if (!current) throw new Error(`Duplicate candidate #${candidateId} not found`)
      if (current.status === status) throw new Error(`Candidate #${candidateId} is already ${status}`)
      ...
      const event = db.insert(duplicateCandidateEvents)
        .values({
          candidateId,
    ...
    merge(primaryObservationId: number, duplicateObservationId: number, options: MergeOptions = {}): MergeResult {
      if (primaryObservationId === duplicateObservationId) {
        throw new Error('Cannot merge observation into itself')
      }

      return db.transaction((tx) => {
        const primaryRow = tx.select().from(observations).where(eq(observations.id, primaryObservationId)).get()
        const duplicateRow = tx.select().from(observations).where(eq(observations.id, duplicateObservationId)).get()
        ...
        const mergedConfidence = mergeConfidence(primary.confidence, duplicate.confidence, confidenceStrategy)
        ...
        const updatedPrimaryRow = tx.update(observations)
          .set({
            description,
            confidence: mergedConfidence,
            frequency: mergedFrequency,
        ...
        tx.insert(observationStatusEvents).values({
        ...
        tx.delete(observationRelations)
          .where(sql`${observationRelations.fromObservationId} = ${observationRelations.toObservationId}`)
          .run()

        const relationRows = tx.select().from(observationRelations)
          .where(or(
        ...
        for (const relationRow of relationRows) {
          ...
          if (seen.has(key)) {
            tx.delete(observationRelations).where(eq(observationRelations.id, relationRow.id)).run()
            continue
          }
          seen.add(key)
        }

        const duplicateMemberships = tx.select().from(clusterMembers)
          .where(eq(clusterMembers.observationId, duplicateObservationId))
          .all()
        for (const member of duplicateMemberships) {
          const existingPrimaryMembership = tx.select().from(clusterMembers)
            .where(and(
        ...
        const existingRelation = tx.select().from(observationRelations)
          .where(and(
        ...
        const relationRow = existingRelation
          ? tx.update(observationRelations)
            ...
            .returning()
            .get()
          : tx.insert(observationRelations)
            .values({
        ...
        const mergeRow = tx.insert(observationMerges).values({
        ...
        const relatedCandidates = tx.select().from(duplicateCandidates)
          .where(and(
        ...
        for (const candidate of relatedCandidates) {
          if (candidate.status !== 'resolved') {
            ...
            tx.insert(duplicateCandidateEvents).values({
        ...
        const mergedRow = tx.select().from(observations).where(eq(observations.id, duplicateObservationId)).get()
        if (!mergedRow) throw new Error(`Observation #${duplicateObservationId} not found after merge`)
