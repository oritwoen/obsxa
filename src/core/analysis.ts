...
export function createAnalysisStore(db: ObsxaDB) {
  return {
    stats(projectId: string): ProjectStats {
      const rows = db.select().from(observations).where(eq(observations.projectId, projectId)).all()
      ...
      const avgConfidence = total > 0
        ? Math.round(rows.reduce((sum, row) => sum + row.confidence, 0) / total)
        : 0

      const totalClusters = db.select({ id: clusters.id }).from(clusters)
    ...
    frequent(projectId: string): Observation[] {
      return db.select().from(observations)
        .where(and(eq(observations.projectId, projectId), notInArray(observations.frequency, [0, 1])))
        .all()
        .map(toObservation)
    ...
    isolated(projectId: string): Observation[] {
      const projectObservations = db.select({ id: observations.id }).from(observations)
    ...
    convergent(projectId: string): Observation[] {
      const allObs = db.select().from(observations).where(eq(observations.projectId, projectId)).all()
      if (allObs.length === 0) return []

      const byId = new Map(allObs.map(o => [o.id, o]))
      const ids = allObs.map(o => o.id)

      const supports = db.select().from(observationRelations)
        .where(and(
    ...
    triage(projectId: string, limit = 25, sort: 'triage' | 'recent' = 'triage'): TriageRow[] {
      const activeRows = db.select().from(observations)
      ...
      const relations = db.select().from(observationRelations)
        .where(and(
          inArray(observationRelations.toObservationId, ids),
          or(
            eq(observationRelations.type, 'supports'),
      ...
      for (const relation of relations) {
        if (relation.type === 'supports') {
          supportsCount.set(relation.toObservationId, (supportsCount.get(relation.toObservationId) ?? 0) + 1)
        }
        if (relation.type === 'contradicts') {
          contradictsCount.set(relation.toObservationId, (contradictsCount.get(relation.toObservationId) ?? 0) + 1)
        }
      }
