...
export function createObservationStore(db: ObsxaDB) {
  return {
    add(input: AddObservation): Observation {
      ...
      const triageScore = computeTriageScore({ confidence, evidenceStrength, novelty, uncertainty, frequency: 1 })
    ...
    addMany(records: ObservationImportRecord[]): Observation[] {
      return records.map((record) => {
        ...
        if (record.status === 'dismissed') {
          return this.dismiss(created.id, { reasonCode: 'manual_review', reasonNote: 'Imported as dismissed' })
        }
        if (record.status === 'archived') {
          return this.archive(created.id, { reasonCode: 'manual_review', reasonNote: 'Imported as archived' })
        }
    ...
    list(
      ...
      return db.select().from(observations).where(and(...conditions)).all().map(toObservation)
    },

    transitions(observationId: number): ObservationTransition[] {
      return db.select().from(observationStatusEvents)
    ...
    edits(observationId: number): ObservationEdit[] {
      return db.select().from(observationEdits)
    ...
    update(id: number, fields: UpdateObservation): Observation {
      ...
      if (fields.title !== undefined) { track('title', current.title, fields.title); values.title = fields.title }
      if (fields.description !== undefined) { track('description', current.description, fields.description); values.description = fields.description }
      if (fields.type !== undefined) { track('type', current.type, fields.type); values.type = fields.type }
      if (fields.source !== undefined) { track('source', current.source, fields.source); values.source = fields.source }
      if (fields.sourceType !== undefined) { track('sourceType', current.sourceType, fields.sourceType); values.sourceType = fields.sourceType }
      if (fields.confidence !== undefined) { const v = clampPercent(fields.confidence, current.confidence); track('confidence', current.confidence, v); values.confidence = v }
      if (fields.tags !== undefined) { const v = JSON.stringify(fields.tags); track('tags', current.tags, v); values.tags = v }
      if (fields.data !== undefined) { track('data', current.data, fields.data); values.data = fields.data }
      if (fields.context !== undefined) { track('context', current.context, fields.context); values.context = fields.context }
      if (fields.capturedAt !== undefined) { const v = parseDate(fields.capturedAt); track('capturedAt', current.capturedAt?.getTime() ?? null, v?.getTime() ?? null); values.capturedAt = v }
      if (fields.sourceRef !== undefined) { track('sourceRef', current.sourceRef, fields.sourceRef); values.sourceRef = fields.sourceRef }
      if (fields.collector !== undefined) { track('collector', current.collector, fields.collector); values.collector = fields.collector }
      if (fields.inputHash !== undefined) { track('inputHash', current.inputHash, fields.inputHash); values.inputHash = fields.inputHash }
      if (fields.evidenceStrength !== undefined) { const v = clampPercent(fields.evidenceStrength, current.evidenceStrength); track('evidenceStrength', current.evidenceStrength, v); values.evidenceStrength = v }
      if (fields.novelty !== undefined) { const v = clampPercent(fields.novelty, current.novelty); track('novelty', current.novelty, v); values.novelty = v }
      if (fields.uncertainty !== undefined) { const v = clampPercent(fields.uncertainty, current.uncertainty); track('uncertainty', current.uncertainty, v); values.uncertainty = v }
      if (fields.reproducibilityHint !== undefined) { track('reproducibilityHint', current.reproducibilityHint, fields.reproducibilityHint); values.reproducibilityHint = fields.reproducibilityHint }

      const nextConfidence = (values.confidence as number | undefined) ?? current.confidence
      const nextEvidence = (values.evidenceStrength as number | undefined) ?? current.evidenceStrength
      ...
      const row = db.update(observations).set(values).where(eq(observations.id, id)).returning().get()

      for (const edit of edits) {
        db.insert(observationEdits).values({
          observationId: id,
          field: edit.field,
    ...
    dismiss(id: number, transition: TransitionObservation): Observation {
      ...
      if (existing.status !== 'active') {
        throw new Error(`Observation #${id} must be active to dismiss (current: ${existing.status})`)
      }

      const row = db.update(observations)
        .set({
          status: 'dismissed',
      ...
      db.insert(observationStatusEvents).values({
        observationId: id,
    ...
    archive(id: number, transition: TransitionObservation): Observation {
      ...
      if (!existing) throw new Error(`Observation #${id} not found`)
      if (existing.status !== 'active') {
        throw new Error(`Observation #${id} must be active to archive (current: ${existing.status})`)
      }
    ...
    promote(id: number, hypothesisRef: string): Observation {
      ...
      if (!existing) throw new Error(`Observation #${id} not found`)
      if (existing.status !== 'active') {
        throw new Error(`Observation #${id} must be active to promote (current: ${existing.status})`)
      }
