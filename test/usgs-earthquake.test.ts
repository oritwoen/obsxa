...
const EARTHQUAKES = [
  ...
  { id: 'us7000qdbn', mag: 5.6, place: '42 km SSE of Hirara, Japan', time: 1741374737490, sig: 482, nst: 119, rms: 0.72, gap: 28, lon: 125.4319, lat: 24.4269, depth: 25.92 },
  { id: 'us7000qdbx', mag: 5.1, place: '33 km SSE of Hirara, Japan', time: 1741376102849, sig: 400, nst: 98, rms: 0.65, gap: 30, lon: 125.3947, lat: 24.5026, depth: 23.1 },
  { id: 'us7000qdgh', mag: 5.0, place: '38 km S of Hirara, Japan', time: 1741391426573, sig: 385, nst: 87, rms: 0.70, gap: 32, lon: 125.2781, lat: 24.4412, depth: 28.5 },

  // Alaska / Attu — cluster of 3
  { id: 'us7000qd6f', mag: 5.3, place: '104 km SSW of Attu Station, Alaska', time: 1741340917283, sig: 432, nst: 132, rms: 0.58, gap: 45, lon: 172.5381, lat: 51.8632, depth: 18.0 },
  { id: 'us7000qd8c', mag: 5.1, place: '110 km S of Attu Station, Alaska', time: 1741354221010, sig: 400, nst: 110, rms: 0.61, gap: 48, lon: 173.0214, lat: 51.8044, depth: 22.4 },
  { id: 'us7000qdza', mag: 5.0, place: '98 km SSW of Attu Station, Alaska', time: 1741440012000, sig: 385, nst: 90, rms: 0.55, gap: 50, lon: 172.8, lat: 51.95, depth: 16.3 },

  // Russia / Kamchatka — cluster of 2
  { id: 'us7000qdcj', mag: 5.2, place: '83 km SSE of Vilyuchinsk, Russia', time: 1741382461893, sig: 416, nst: 142, rms: 0.78, gap: 35, lon: 158.6713, lat: 52.2118, depth: 56.83 },
  { id: 'us7000qdkz', mag: 5.4, place: '90 km SE of Vilyuchinsk, Russia', time: 1741415000000, sig: 449, nst: 128, rms: 0.80, gap: 38, lon: 158.9, lat: 52.15, depth: 48.0 },

  // Indonesia — cluster of 2
  { id: 'us7000qdgb', mag: 6.0, place: '154 km N of Manokwari, Indonesia', time: 1741389936613, sig: 554, nst: 145, rms: 0.81, gap: 22, lon: 133.7991, lat: -0.2138, depth: 10.0 },
  { id: 'us7000qdx2', mag: 5.3, place: '120 km NNW of Manokwari, Indonesia', time: 1741430000000, sig: 432, nst: 100, rms: 0.76, gap: 28, lon: 133.5, lat: -0.1, depth: 15.0 },

  // Tonga — standalone
  { id: 'us7000qds1', mag: 5.2, place: '129 km ESE of Neiafu, Tonga', time: 1741410530610, sig: 416, nst: 85, rms: 0.92, gap: 55, lon: -173.0492, lat: -18.9512, depth: 210.63 },

  // Italy — standalone (notable: shallow, high significance)
  { id: 'us7000qdyv', mag: 6.0, place: '12 km W of Anacapri, Italy', time: 1741436505100, sig: 554, nst: 160, rms: 0.68, gap: 18, lon: 14.1183, lat: 40.5444, depth: 8.0 },

  // Philippines — standalone
  { id: 'us7000qdm5', mag: 5.5, place: '5 km NNE of Magsaysay, Philippines', time: 1741400000000, sig: 465, nst: 75, rms: 0.88, gap: 40, lon: 125.51, lat: 6.83, depth: 55.0 },

  // Chile — standalone
  { id: 'us7000qdp3', mag: 5.1, place: '45 km NW of Calama, Chile', time: 1741405000000, sig: 400, nst: 65, rms: 0.60, gap: 60, lon: -68.95, lat: -22.25, depth: 120.0 },
] as const
...
describe('usgs earthquake integration', () => {
  ...
  it('ingests 14 earthquakes as observations', () => {
    ...
    const observations = obsxa.observation.addMany(
      EARTHQUAKES.map(eq => ({
        ...
        data: JSON.stringify({ lon: eq.lon, lat: eq.lat, depth: eq.depth, mag: eq.mag, gap: eq.gap, rms: eq.rms }),
        context: JSON.stringify({ network: 'us', reviewStatus: 'reviewed', magType: 'mww', stations: eq.nst }),
  ...
  it('creates relations between earthquakes in same region', () => {
    ...
    obsxa.relation.add({ fromObservationId: japan[1]!.id, toObservationId: japan[0]!.id, type: 'same_signal_as', confidence: 90, notes: 'Aftershock sequence near Hirara' })
    obsxa.relation.add({ fromObservationId: japan[2]!.id, toObservationId: japan[0]!.id, type: 'same_signal_as', confidence: 85, notes: 'Same fault system' })

    // Alaska cluster: 3 quakes near Attu — supports
    const alaska = added.slice(3, 6)
    obsxa.relation.add({ fromObservationId: alaska[1]!.id, toObservationId: alaska[0]!.id, type: 'supports', confidence: 80 })
    obsxa.relation.add({ fromObservationId: alaska[2]!.id, toObservationId: alaska[0]!.id, type: 'supports', confidence: 75 })
    ...
    obsxa.relation.add({ fromObservationId: italy.id, toObservationId: chile.id, type: 'contradicts', notes: 'Shallow crustal vs deep subduction' })
  ...
  it('analysis: convergent detects multi-source support', () => {
    ...
    obsxa.relation.add({ fromObservationId: added[4]!.id, toObservationId: added[3]!.id, type: 'supports' })
    obsxa.relation.add({ fromObservationId: added[5]!.id, toObservationId: added[3]!.id, type: 'supports' })
  ...
  it('analysis: isolated finds standalone quakes', () => {
    ...
    obsxa.relation.add({ fromObservationId: added[1]!.id, toObservationId: added[0]!.id, type: 'same_signal_as' })
    obsxa.relation.add({ fromObservationId: added[2]!.id, toObservationId: added[0]!.id, type: 'same_signal_as' })
  ...
  it('lifecycle: dismiss, archive, promote', () => {
    ...
    const dismissed = obsxa.observation.dismiss(added[2]!.id, { reasonCode: 'noise', reasonNote: 'Below signal threshold' })
    expect(dismissed.status).toBe('dismissed')

    // Archive for later reference
    const archived = obsxa.observation.archive(added[3]!.id, { reasonCode: 'manual_review', reasonNote: 'Needs geological review' })
  ...
  it('end-to-end: full pipeline from ingest to triage', () => {
    // 1. Create project
    const project = obsxa.project.add({ id: 'seismic-e2e', name: 'E2E Seismic Monitor', description: 'Full pipeline test with USGS data' })
    ...
    obsxa.relation.add({ fromObservationId: added[1]!.id, toObservationId: added[0]!.id, type: 'same_signal_as', confidence: 90 })
    obsxa.relation.add({ fromObservationId: added[2]!.id, toObservationId: added[0]!.id, type: 'same_signal_as', confidence: 85 })
    // Alaska seismicity
    obsxa.relation.add({ fromObservationId: added[4]!.id, toObservationId: added[3]!.id, type: 'supports', confidence: 80 })
    obsxa.relation.add({ fromObservationId: added[5]!.id, toObservationId: added[3]!.id, type: 'supports', confidence: 75 })
    // Kamchatka pair
    obsxa.relation.add({ fromObservationId: added[7]!.id, toObservationId: added[6]!.id, type: 'supports', confidence: 82 })
    // Indonesia pair
    obsxa.relation.add({ fromObservationId: added[9]!.id, toObservationId: added[8]!.id, type: 'supports', confidence: 78 })
    ...
    obsxa.observation.dismiss(added[10]!.id, { reasonCode: 'noise', reasonNote: 'Isolated deep event' })
